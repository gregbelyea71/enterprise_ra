...
var EditableDatagrid = function (props) {
    ...
    if (rest.loaded && (rest.ids.length === 0 || rest.total === 0)) {
        return hasStandaloneCreateForm ? (React.createElement(React.Fragment, null,
            React.createElement(buttons_1.CreateButton, { onClick: openStandaloneCreateForm, resource: resource }),
            datagridBody)) : (datagridBody);
    }
    return (React.createElement(react_admin_1.Datagrid, __assign({ body: datagridBody, resource: resource }, rest),
        children,
        actions !== false
            ? actions || (React.createElement(ActionsColumn_1.ActionsColumn
            // Datagrid cells supports the label prop which accept a React element.
            , { 
                // Datagrid cells supports the label prop which accept a React element.
                label: hasStandaloneCreateForm ? (React.createElement(buttons_1.InlineCreateButton, { onClick: openStandaloneCreateForm })) : (''), mutationMode: mutationMode, undoable: undoable, noDelete: noDelete, redirect: isInListView ? 'list' : false }))
            : null));
};
