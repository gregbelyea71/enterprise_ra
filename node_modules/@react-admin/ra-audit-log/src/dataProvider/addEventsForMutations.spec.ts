...
describe('addEventsForMutations', () => {
    ...
    test.each([
        ...
        'should create a new log for each record when calling dataProvider.%s and no options have been provided',
        async (method, payload) => {
            const dataProvider = addEventsForMutations(
                baseDataProvider,
                authProvider
            ...
            expect(baseDataProvider.create).toHaveBeenCalledTimes(2);
            const { ids, ...payloadRest } = payload;
            for (let index = 0; index < ids.length; index++) {
                const id = ids[index];
                const params = baseDataProvider.create.mock.calls[index][1];

                expect(params.data.date).toBeDefined();
                expect(params.data.author).toEqual(author);
                expect(params.data.resource).toEqual('products');
                expect(params.data.action).toEqual(method);
                expect(params.data.payload).toEqual({
                    id,
                    ...payloadRest,
                });
            }
    ...
    test('should use a custom matcher function to determine whether to audit a dataProvider call', async () => {
        ...
        expect(baseDataProvider.create).toHaveBeenCalled();
        const params = baseDataProvider.create.mock.calls[0][1];
